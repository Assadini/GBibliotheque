...
//                + " AND ";
    ...
    public void setId(int id) throws GbException {
        if (id <= 0) {
            throw new GbException("Identifiant Reservation non valide !");
        }
        this.id = id;
    }
    ...
    public void reserver(Usager usager, Oeuvre oeuvre, Date dateJour) throws GbException {
        Reservation reservation = findByReservation(usager, oeuvre);
        if (null != reservation) {
            throw new GbException("Vous avez déjà réservé l'oeuvre '" + oeuvre.getTitre()
        ...
        try {
            preparedStatement = initialiseRequetePreparee(Connect.getConnection(), SQL_INSERT,
                    oeuvre.getId(), usager.getId(),
                    formatedDateJour, null);
            int statut = preparedStatement.executeUpdate();
            if (statut == 0) {
                throw new GbException("Echec lors de l'enregistrement de la réservation");
            }
        } catch (SQLException e) {
            throw new GbException("Erreurs lors de l'enregistrement de la réservation", e.getCause());
        } catch (GbException e) {
            throw new GbException("Erreurs lors de l'enregistrement de la réservation ", e.getCause());
        } finally {
    ...
    public void annuler(Usager usager, Oeuvre oeuvre, Reservation reservation) throws GbException {
        ...
        try {
            ...
            preparedStatement = initialiseRequetePreparee(Connect.getConnection(), SQL_UPDATE_RES,
                    formatedDateAnnulation, oeuvre.getId(),
                    usager.getId());
            int statut = preparedStatement.executeUpdate();
            if (statut == 0) {
                throw new GbException("Echec de l'annulation de la réservation");
            }
        } catch (SQLException | GbException e) {
            throw new GbException("Erreurs lors de  l'annulation de la réservation", e.getCause());
        } finally {
            closeStatement(preparedStatement);
        }
    }

    public Reservation findById(int id) throws GbException {
    ...
    public ArrayList<Reservation> findByDateReservaton(Date dateRes) throws GbException {
    ...
    public Reservation findByReservation(Usager usager, Oeuvre oeuvre) throws GbException {
    ...
    public ArrayList<Reservation> findByReservation(Oeuvre oeuvre) throws GbException {
    ...
    private ArrayList<Reservation> find(String sql, Object... objets) throws GbException {
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
        ArrayList<Reservation> listReservations = new ArrayList<>();

        try {
            preparedStatement = initialiseRequetePreparee(Connect.getConnection(), sql, objets);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                listReservations.add(mappingReservation(resultSet));
            }
        } catch (SQLException e) {
            throw new GbException("Aucun enregistrement trouvé " + e.getMessage());
        } finally {
